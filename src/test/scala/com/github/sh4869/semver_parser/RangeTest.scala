...
class RangeTest extends AnyFunSuite with Matchers {
  ...
  test("specify") {
    ...
    assert(parse("v1.0.0") exists (r => r.valid(SemVer("1.0.0")) && r.invalid(x)))
    assert(parse("0.4.0-alpha.04512140").isDefined)
  }
  ...
  test("caret range") {
    ...
    assert(parse("^v1.0.0") exists (r => check(r, "1.1.1", "2.0.0")))
  }
